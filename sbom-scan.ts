...
const entries = lines.map(parseLine).filter((e): e is ParsedEntry => e !== null);
...
async function fetchSbom(owner: string, repoName: string): Promise<SbomData> {
  ...
  if (!m) throw new Error(`Unexpected generate-report response: ${JSON.stringify(genRes.data)}`);
  const uuid = m[1];
...
await pool(reposToScan, CONCURRENCY, async (repo) => {
  ...
  try {
    ...
    const pkgs: SpdxPackage[] = Array.isArray(sbom?.packages) ? sbom.packages : [];
    console.log(`Found ${pkgs.length} packages in repository. Checking for matches with affected packages`);
    ...
    for (const p of pkgs) {
      ...
      if (semver.lte(nv.version, maxAllowed)) {
        console.log(`Package ${String(p.name)}@${String(p.versionInfo)} matched with an affected version`);
...
for (const [pair, setRepos] of affectedMap.entries()) {
  ...
  matches.push({ package: pkg, version: ver, repositories: Array.from(setRepos).sort() });
}
matches.sort((a, b) =>
  a.package === b.package
...
console.log(`Wrote ${OUT} with ${matches.length} distinct package@version matches across ${reposToScan.length} repos.`);
